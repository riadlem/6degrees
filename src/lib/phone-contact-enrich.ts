...
export async function enrichContactsFromPhoneBook(
  ...
  if (phoneContacts.length === 0) return { enriched: 0, phones: 0, emails: 0, photos: 0, linkedinUrls: 0 }
  ...
  const nameIndex = new Map<string, string[]>() // "first|last" → contactId[]
  for (const c of allContacts) {
  ...
  for (const pc of phoneContacts) {
    ...
    if (pc.phone && !contact.phoneNumber) {
      ...
      contact.phoneNumber = pc.phone // update in-memory to avoid double-enriching
    }
    ...
    if (pc.email && !contact.emailAddress) {
      ...
      emailIndex.set(normalEmail, contactId) // update index
    }
  ...
  return {
    enriched,
    phones: phoneUpdates.length,
    emails: emailUpdates.length,
    photos: photoUpdates.length,
    linkedinUrls: linkedinUpdates.length,
  }
}
