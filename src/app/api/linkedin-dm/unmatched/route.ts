...
export async function GET(req: Request) {
  ...
  // Group unmatched LinkedIn DM messages by chatName
  const grouped = await prisma.linkedInDMMessage.groupBy({
    by: ["chatName"],
  ...
  const chats = await Promise.all(
    paginated.map(async (g) => {
      ...
      const firstMsg = await prisma.linkedInDMMessage.findFirst({
        where: { userId, chatName: g.chatName },
      ...
      return {
        ...
        profileUrl: firstMsg?.profileUrl ?? null,
        suggestions,
      }
    }),
  )
